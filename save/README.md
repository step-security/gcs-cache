...
steps:
  ...
  - uses: step-security/gcs-cache/save@v1
    id: cache
    with:
...
```yaml
uses: step-security/gcs-cache/restore@v1
id: restore-cache
with:
...
For such use-cases, users now have the ability to use the `step-security/gcs-cache/save` action to save the cache by using an [`always()`](https://docs.github.com/actions/writing-workflows/choosing-what-your-workflow-does/expressions#always) condition.
This way the cache will always be saved if generated, or a warning will be generated that nothing is found on the cache path. Users can also use the `if` condition to only execute the `step-security/gcs-cache/save` action depending on the output of previous steps. This way they get more control of when to save the cache.
